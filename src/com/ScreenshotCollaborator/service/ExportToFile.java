...
public abstract class ExportToFile {
	...
	/**
	 * Method to return the list of batch folder present in the sourece
	 * directory.
	 * 
	 * @author Arpit K Tiwari
	 * @version 1.0
	 * @since 2017-08-22
	 * @param srcFolder
	 * @return File[] List of Batch folder in the source directory.
	 */

	public File[] getFilteredImgFoldersName(String srcFolder) {
		File baseFolder = new File(srcFolder);
		FilenameFilter nameFilter = new FilenameFilter() {
			@Override
			public boolean accept(File file, String name) {
				File tmpFile = new File(file.getAbsoluteFile() + File.separator + name);
				if (tmpFile.isDirectory() && name.startsWith(Constants.BATCH)) {
					return true;
				} else {
					return false;
				}
		...
		return baseFolder.listFiles(nameFilter);
	}
